...
class DDPMWrapperE2EKD_Scratch(pl.LightningModule):
    def __init__(
        self,
        online_network,
        target_network,
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.conditional:
            ...
            origin_mu, origin_logvar = self.origin_vae.encode(x * 0.5 + 0.5)

            z = self.vae.reparameterize(mu, logvar)
            origin_z = self.origin_vae.reparameterize(origin_mu, origin_logvar)

            cond = self.vae.decode(z)
            cond = 2 * cond - 1

            origin_cond = self.origin_vae.decode(origin_z)
            origin_cond = 2 * origin_cond - 1

            kd_loss = self.mse_loss(mu, origin_mu) + self.mse_loss(logvar, origin_logvar) + self.compute_kl(mu, logvar)
            kd_loss += self.mse_loss(cond, x) + self.mse_loss(cond, origin_cond)
        ...
        # Compute loss
        loss = self.criterion(eps, eps_pred) + kd_loss
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.Adam(
            list(self.online_network.decoder.parameters())+list(self.vae.parameters()), lr=self.lr
        )
